...
with tabs[4]:
    ...
    if use_ml and 'ml_weights' in st.session_state:
        ...
        col3.metric("ROE Weight", f"{w_roe:.0%}")
        col4.metric("Leverage Weight", f"{w_debt:.0%}")
        col5.metric("Valuation Weight", f"{w_val:.0%}")
        
    else:
        col1, col2, col3, col4, col5 = st.columns(5)
        with col1: w_ret = st.slider("5Y CAGR (Growth)", 0.0, 1.0, 0.2)
    ...
    fig=px.bar(score.reset_index(names="Company"),x="Company",y="Composite / 100",color="Company",color_discrete_map=COLORS,title="Quantitative Scorecard")
